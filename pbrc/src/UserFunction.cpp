		...
		shared_ptr<SensorEvent> sensor_ev)
{
	...
	if (dvs_ev && (dvs_ev->p == 0)) {
		...
		// preferred period
		constexpr int TIME_JITTER = 10;
		if (timeDiff < TIME_JITTER) {

			// temporal weight, linear
			float weightT = std::abs((15.f - float(timeDiff)) / float(TIME_JITTER));
