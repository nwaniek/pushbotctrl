	    ...
	    shared_ptr<SensorEvent> sensor_ev)
{
	...
	if (dvs_ev && (dvs_ev->p == 0)) {
		...
		// LED window
		if (timeDiff < 200) {
			float weightT = std::abs((200.f - float(timeDiff)) / 200.f);
			weightT *= weightT * weightT;

			float weightX = std::abs((x - data->tracker.x) / 128.f);
			weightX *= weightX * weightX;

			float weightY = std::abs((y - data->tracker.y) / 128.f);
			weightY *= weightY * weightY;

			// combine
			weightX = tao * weightX * weightT;
			weightY = tao * weightY * weightT;

			// std::cout << weightX << ", " << weightY << "\n";
	...
	if (!dvs_ev && !sensor_ev) {
		...
		if (data->i > 5) {
			/*
			// the target tells where we want to see the LD
			Vec2f target{64.0, 40.0};
			float xDrive = target.x - data->tracker.x;
			float yDrive = data->tracker.y - target.y;
			control->drive(-xDrive / 128.f, yDrive / 128.f);
			*/
		}

		// reset certainty
		data->i = 0;
		std::cout << int(control->id()) << ": "
			<< data->tracker.x << ", " << data->tracker.y << std::endl;
	}
